...
using
	...
	CPSC131::BookStore::InsufficientInventory
	;
...
int main()
{
	...
	PuhPTests::Random::fixed_seed = 1981928872;
...
void testIterators(Tests& tests)
{
	...
	t->assertTypedException(
		[t]()
		{
			...
			auto thisShouldNotWork = *itr;
			thisShouldNotWork = thisShouldNotWork;
			
		}, typeid(std::out_of_range), 1, "Attempting to dereference an \"end\" iterator should throw an exception"
	);
		...
		{
			{
				...
				//	Addition operator shouldn't change 'this'
				{
					auto itr_old = itr;
					auto itr_temp = itr + half;
					itr_temp = itr_temp;
					auto itr_new = itr;
					t->assertTrue
					(
						itr_new.getCursor() == itr_old.getCursor(), 1,
						"Iterator addition operator shouldn't change what \"this\" is pointing to"
					);
				}
				
				//	Subtraction operator shouldn't change 'this'
				{
					auto itr_at_half = itr + half;
					auto itr_old = itr_at_half;
					auto itr_temp = itr_at_half - (half / 2);
					itr_temp = itr_temp;
					auto itr_new = itr_at_half;
					t->assertTrue
					(
						itr_new.getCursor() == itr_old.getCursor(), 1,
						"Iterator subtraction operator shouldn't change what \"this\" is pointing to"
					);
				}
			}
	...
	t->assertNoException(
		...
		}, 1, "Iterator addition/subtraction assignment operators"
	);
...
void testStockSales(Tests& tests)
{
	...
	t->assertTypedException(
		...
		}, typeid(InsufficientInventory), 1, "Should see an exception if we try to oversell a book (ISBN overload)."
	);
	
	//	Check the other overload of sellToCustomer.
	//	It should also throw if we try to oversell.
	//	(some students may not have realized its easier to do wrapper-pattern here,
	//	 and will have written both overloads separately)
	t->assertTypedException(
		[t]()
		{
			//
			...
			//store.printInventory();
			auto itr_book = store.findBook("1435769244");
			store.sellToCustomer(*itr_book, 2000, 6);
		}, typeid(InsufficientInventory), 1, "Should see an exception if we try to oversell a book (Book overload)."
	);
